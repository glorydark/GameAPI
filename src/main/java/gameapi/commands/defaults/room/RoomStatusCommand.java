                ...
                if (!rooms.isEmpty()) {
                    for (Room room : rooms) {
                        if (!room.isAllowedToStart()) {
                            builder.append(GameAPI.getLanguage().getTranslation(commandSender, "command.status.show.tag.need_start_pass", room.getRoomName(), room.getCurrentRoomStatus().toString(), room.getPlayers().size(), room.getMaxPlayer(), room.getMinPlayer()))
                                    .append("\n");
                        } else {
                            builder.append(GameAPI.getLanguage().getTranslation(commandSender, "command.status.show.tag.common", room.getRoomName(), room.getCurrentRoomStatus().toString(), room.getPlayers().size(), room.getMaxPlayer(), room.getMinPlayer()))
                                    .append("\n");
                        }
